...
vid = YouTube('https://www.youtube.com/watch?v=00gegkMZpvs&t')
...
if selected.is_progressive:
	selected.download(output_path=path)
else:
	...
	argfunc = "output_path='./output'"
	job_thread1 = threading.Thread(target=selected.download, kwargs={'output_path':path})
	job_thread2 = threading.Thread(target=selected_aud.download, kwargs={'output_path':path})
	job_thread1.start()
	job_thread2.start()